...
public abstract class DeepLinkRoutingValidator {
    private static WeakReference current_activity_reference;

    public static void validate(WeakReference weakReference) {
        ...
        if (!TextUtils.isEmpty(getLatestReferringLink()) && weakReference != null) {
            ...
            } else if (latestReferringParams.optBoolean("bnc_validate")) {
    ...
    private static void validateDeeplinkRouting(final JSONObject jSONObject) {
        if (current_activity_reference.get() != null) {
            new AlertDialog.Builder((Context) current_activity_reference.get(), 16974374).setTitle("Branch Deeplinking Routing").setMessage("Good news - we got link data. Now a question for you, astute developer: did the app deep link to the specific piece of content you expected to see?").setPositiveButton("Yes", new DialogInterface.OnClickListener() {

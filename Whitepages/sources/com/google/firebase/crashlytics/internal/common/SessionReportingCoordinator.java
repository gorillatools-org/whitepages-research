...
public class SessionReportingCoordinator {
    ...
    private void persistEvent(Throwable th, Thread thread, String str, EventMetadata eventMetadata, boolean z) {
        ...
        if (!z) {
            this.crashlyticsWorkers.diskWrite.submit((Runnable) new SessionReportingCoordinator$$ExternalSyntheticLambda7(this, addMetaDataToEvent, eventMetadata, equals));
        } else {
    ...
    private static CrashlyticsReport.ApplicationExitInfo convertApplicationExitInfo(ApplicationExitInfo applicationExitInfo) {
        String str = null;
        try {
            InputStream m = applicationExitInfo.getTraceInputStream();
            if (m != null) {
    ...
    private ApplicationExitInfo findRelevantApplicationExitInfo(String str, List<ApplicationExitInfo> list) {
        ...
        for (ApplicationExitInfo m : list) {
            ApplicationExitInfo m2 = ForceStopRunnable$$ExternalSyntheticApiModelOutline1.m(m);
            if (m2.getTimestamp() < startTimestampMillis) {
                return null;
            }
            if (m2.getReason() == 6) {
                return m2;

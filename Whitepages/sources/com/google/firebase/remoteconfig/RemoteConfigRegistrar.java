...
public class RemoteConfigRegistrar implements ComponentRegistrar {
    ...
    public List<Component<?>> getComponents() {
        ...
        return Arrays.asList(new Component[]{Component.builder(RemoteConfigComponent.class, (Class<? super T>[]) new Class[]{FirebaseRemoteConfigInterop.class}).name(LIBRARY_NAME).add(Dependency.required((Class<?>) Context.class)).add(Dependency.required((Qualified<?>) qualified)).add(Dependency.required((Class<?>) FirebaseApp.class)).add(Dependency.required((Class<?>) FirebaseInstallationsApi.class)).add(Dependency.required((Class<?>) AbtComponent.class)).add(Dependency.optionalProvider((Class<?>) AnalyticsConnector.class)).factory(new RemoteConfigRegistrar$$ExternalSyntheticLambda0(qualified)).eagerInDefaultApp().build(), LibraryVersionComponent.create(LIBRARY_NAME, BuildConfig.VERSION_NAME)});
    }

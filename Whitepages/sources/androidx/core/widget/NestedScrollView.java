...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild {
    ...
    private float getSplineFlingDistance(int i) {
        double log = Math.log((double) ((((float) Math.abs(i)) * 0.35f) / (this.mPhysicalCoeff * 0.015f)));
        float f = DECELERATION_RATE;
        return (float) (((double) (this.mPhysicalCoeff * 0.015f)) * Math.exp((((double) f) / (((double) f) - 1.0d)) * log));
    }
    ...
    public void fling(int i) {
        if (getChildCount() > 0) {
            this.mScroller.fling(getScrollX(), getScrollY(), 0, i, 0, 0, ExploreByTouchHelper.INVALID_ID, Integer.MAX_VALUE, 0, 0);

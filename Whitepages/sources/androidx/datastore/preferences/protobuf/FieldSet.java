...
final class FieldSet {
    ...
    public FieldSet clone() {
        ...
        for (int i = 0; i < numArrayEntries; i++) {
            Map.Entry arrayEntryAt = this.fields.getArrayEntryAt(i);
            MediaControllerCompat$MediaControllerImplApi21$ExtraBinderRequestResultReceiver$$ExternalSyntheticThrowCCEIfNotNull0.m(arrayEntryAt.getKey());
            newFieldSet.setField((FieldDescriptorLite) null, arrayEntryAt.getValue());
        }
        for (Map.Entry entry : this.fields.getOverflowEntries()) {
            ...
            newFieldSet.setField((FieldDescriptorLite) null, entry.getValue());
        }
    ...
    public int getSerializedSize() {
        ...
        for (int i2 = 0; i2 < numArrayEntries; i2++) {
            ...
            MediaControllerCompat$MediaControllerImplApi21$ExtraBinderRequestResultReceiver$$ExternalSyntheticThrowCCEIfNotNull0.m(arrayEntryAt.getKey());
            i += computeFieldSize((FieldDescriptorLite) null, arrayEntryAt.getValue());
        }
        for (Map.Entry entry : this.fields.getOverflowEntries()) {
            MediaControllerCompat$MediaControllerImplApi21$ExtraBinderRequestResultReceiver$$ExternalSyntheticThrowCCEIfNotNull0.m(entry.getKey());
            i += computeFieldSize((FieldDescriptorLite) null, entry.getValue());
        }
        return i;
    }
